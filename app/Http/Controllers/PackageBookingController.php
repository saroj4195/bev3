...
class PackageBookingController extends Controller
{
                ...
                foreach ($package_inventory as $key => $package) {
                    ...
                    foreach ($period as $value) {
                        ...
                        if (in_array($index, $black_out)) {
                            $converted_package_inventory[$key]["min_inv"] = 0;
    ...
    public function getSpecificPackageDetailsNew(int $hotel_id, $from_date, $currency_name)
    {
        $from_date = date('Y-m-d', strtotime($from_date));

        if (!$hotel_id) {
            return response()->json(["status" => 0, "message" => "Failed to retrieve details"]);
        }
        ...
        if ($package_inventory->isEmpty()) {
            return response()->json(["status" => 0, "message" => "Package not found"]);
        }

        $package_inventory = $package_inventory->toArray();
        $filter_array = [];
        foreach ($package_inventory as $key => $package) {
            $to_date = date('Y-m-d', strtotime($from_date . '+' . $package['nights'] . ' day'));
        
            if ($package['date_to'] < $to_date) {
                // Unset the value and re-arrange the indices
                unset($package_inventory[$key]);
                continue;
            }
            ...
            $discount_percentage = $package['discount_amount'] / $package['amount'] * 100;
            $package['discount_percentage'] = round($discount_percentage);
            // $package["date_to"] = date('Y-m-d', strtotime($from_date . '+' . $package->nights . ' day'));
            $from_date = date('Y-m-d', strtotime($from_date));

            $package['min_inv'] = 0;
            $package['image'] = isset($image_array[0])?$image_array[0]:[];
            ...
            if ($room_details) {
                $room_amenities = $room_details->room_amenities;
                $amenities_array = explode(',', $room_amenities);
                $amenities_name = DB::table('kernel.room_amenities')->whereIn('room_amenities_id', $amenities_array)->get();
                $ame = [];
                foreach ($amenities_name as $amenities) {
                    $ame[] = array('name' => $amenities->room_amenities_name);
                }

                $package['amenities']  = $ame;
            }
            ...
            $package['total_occupancy'] = $package['base_adult'] + $package['extra_adult'] + $package['base_child'] + $package['extra_child'];


            $package['min_inv'] = $info[0]['no_of_rooms'];

            // Example: Check blackout dates
            $black_out = array_column(json_decode($package['blackout_dates'], true), "date");
            $blackoutCheck = $this->checkBlackoutDates($from_date, $package['date_to'], $black_out);

            if ($blackoutCheck) {
                unset($package_inventory[$key]);
            }

            array_push($filter_array,$package);
        }


        // $package_inventory = array_values($package_inventory);
        // dd($package_inventory);
        // $package_inventory = collect($package_inventory_array);

        if (sizeof($filter_array)>0) {
            return response()->json(["status" => 1, "message" => "Successfully retrieved details", "package_inventory" => $filter_array]);
        } else {
            return response()->json(["status" => 0, "message" => "Package not found"]);
        }
    }

    private function checkBlackoutDates($from_date, $end_date, $black_out_dates)
    {
        $period = new \DatePeriod(
            new \DateTime($from_date),
            new \DateInterval('P1D'),
        ...
        foreach ($period as $value) {
            $index = $value->format('d-m-Y');
            if (in_array($index, $black_out_dates)) {
                return true;
            }
        }
    ...
    public function getPackages(Request $request, $id, $from, $to)
    {
        $package_inventory = [
            [
                "package_id" => 606,
                ...
                "room_type_id" => 11480,
                "user_id" => 0,
                "package_name" => "Matal Package",
                "date_from" => "2023-10-03",
                "date_to" => "2023-10-07",
                "nights" => 3,
                "amount" => 150,
                "discounted_amount" => 50,
                "package_description" => "<p>wewewe<\/p>",
                "package_image" => [
                    "uploads\/1695624491864935.png"
                ],
                "base_adult" => 3,
                "base_child" => 3,
                "extra_adult" => 3,
                ...
                "exclusion" => null,
                "aminities" => [],
                "image_name" => "uploads\/1695624491864935.png",
                "room_type" => "Royal Room",
                "min_inv" => 10,
                "room_size_value" => 100,
                "room_size_unit" => 'sf',
                "bed_type" => 'King',
                "room_view_type" => "sea view",
                "room_occupancy" => 9,
            ],
            [
                "package_id" => 608,
        ...
        if ($package_inventory) {
            $res = array("status" => 1, "data" => $package_inventory, "message" => "successfully retrive details");
            return response()->json($res);
        }
    }
}
