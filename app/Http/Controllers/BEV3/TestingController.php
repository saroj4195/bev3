...
class TestingController extends Controller
{
    ...
    public function bookingsTestNew(string $api_key, Request $request)
    {
        //store the log
    ...
    public function bookingsTestNewold( $api_key, Request $request)
    {
        $logpath = storage_path("logs/prip-cart.log" . date("Y-m-d"));
        $logfile = fopen($logpath, "a+");
        fwrite($logfile, "Processing starts at: " . date("Y-m-d H:i:s") . "\n");
        ...
        if (sizeof($room_details) > 0) {
            ...
            foreach ($room_details as $details) {
                ...
                for ($i = 0; $i < $diff; $i++) {
                    ...
                    foreach ($occupancy as $key => $acc) {
                        ...
                        $base_occupancy = $base_adult + $base_child;
                        $total_occupancy = $adult + $child;
                        $max_occupancy = $room_type_details->max_occupancy;

                        $room_price = $bar_price;
                        if($total_occupancy > $max_occupancy)
                        {
                            return response()->json(array('status' => 0, 'message' => 'Selected occupancy exceeds the maximum allowed occupancy.'));
                        }

                    
                        if ($adult <= $base_adult) {
                            if ($total_occupancy <= $base_occupancy) {
                                $room_price = isset($multiple_occupancy[$total_occupancy-1]) ? $multiple_occupancy[$total_occupancy-1] : $bar_price;
                                $discounted_price = $room_price * $coupons_percentage / 100;
                                $price_after_discount =  $room_price -  $discounted_price;
                            ...
                            $discounted_price = $room_price * $coupons_percentage / 100;
                            $price_after_discount =  $room_price -  $discounted_price;
                            $total_extra_adult_price += $extra_adult * $extra_adult_price;
                        }

                        
                        if ($child > $base_child) {
                            $extra_child = $child - $base_child; //1 //2     
                    
                            if ($base_adult > $adult) {
                                $remaining_adult = $base_adult - $adult; //1
                                $required_child = $child - $base_child; //1 //2
                    
                                if ($remaining_adult >= $required_child) {
                                    $room_price = $bar_price;
                                    $discounted_price = $room_price * $coupons_percentage / 100;
                                    $price_after_discount =  $room_price -  $discounted_price;
                                    $extra_child = 0;
                                    ...
                                    $extra_child = $required_child - $remaining_adult;
                                    $discounted_price = $room_price * $coupons_percentage / 100;
                                    $price_after_discount =  $room_price -  $discounted_price;

                                    $price_after_discount += $extra_child * $extra_child_price;
                                }
                            } else {
                            
                                $extra_child = $child - $base_child;
                                $price_after_discount =  $room_price -  $discounted_price;
                                $total_extra_child_price += $extra_child * $extra_child_price;
                            }
                        }


                        $price_after_discount = $price_after_discount + $total_extra_adult_price + $total_extra_child_price;
                         
                        //Gst calculation
                        if ($hotel_info->is_taxable == 1) {
                            if ($hotel_info->gst_slab == 1) {
                                $price_for_gst_slab = $price_after_discount ;
                                $gst_percentage = $this->checkGSTPercent($price_for_gst_slab);
                            } else {
                                $gst_percentage = $this->checkGSTPercent($price_after_discount);
                            }
                            $room_price_excluding_gst = $price_after_discount ;
            ...
            //     'coupons_percentage' => $coupons_percentage,
            //     'total_paidservices_amount' => $total_paidservices_amount,
    ...
    public function handleIds($cart, $from_date, $to_date, $booking_date, $hotel_id, $user_id, $booking_status,$hotel_info)
    {
        ...
        if ($ids_status) {
            ...
            // dd($ids_data,$customer_data);

            $type = "Bookingjini";
            $last_ids_id = $this->idsService->idsBookingsTest($hotel_id, $type, $ids_data, $customer_data, $booking_status);
            if ($last_ids_id) {
                return $last_ids_id;
            } else {
                return 0;
    ...
    public  function preinvoiceMail($id)
    {
        ...
        $to_email = 'sarojkumarpatel12@gmail.com'; //don't change this
    ...
    public function getCurrentRatesByRoomnRatePlan($hotel_id, $room_type_id, $rate_plan_id, $date_from, $date_to, $baseCurrency, $currency_name)
    {
       ...
       // ->where('status', 1) 
          ->where('stay_start_date', '<=', $date_from)
          ...
          ->get();
            ...
            for ($i = 0; $i < $diff; $i++) {
                ...
                // if($hotel_id == 2600){
                    ...
                    if ($basicPromotions) {
                        foreach($basicPromotions as $promotions){

                            $basicRoomRatePlan = json_decode($promotions->selected_room_rate_plan, true);
                            $blackoutdates = explode(',', $promotions->blackoutdates);
                            // dd($blackoutdates);
    ...
    public function getCurrentRatesByRoomnRatePlanTest($hotel_id, $room_type_id, $rate_plan_id, $date_from, $date_to, $baseCurrency, $currency_name)
    {
        ...
        if (sizeof($rates) > 0) {
            ...
            $applicable_room_and_rate_plan = [];
            $max_discount_basic_promotions = [];

            if(sizeof($basicPromotions)>0){
                foreach($basicPromotions as $basicPromotion){

                    $selected_room_rate_plan = $basicPromotion->selected_room_rate_plan;
                    $selected_room_rate_plan = json_decode($selected_room_rate_plan);

                    foreach ($selected_room_rate_plan as $room) {
                        if ($room->room_type_id == $room_type_id) {
                            if (in_array($rate_plan_id, $room->selected_rate_plans)) {
                                $applicable_room_and_rate_plan []= array('promotion_id'=>$basicPromotion->id, $room_type_id => $rate_plan_id);
                                $max_discount_basic_promotions[]= $basicPromotion->discount_percentage;
                            }
                        }
                    }
                }
            ...
            $max_discount_basic_promotions = max($max_discount_basic_promotions);
            ...
            $max_discount_earlybird_promotions = [];

            if(sizeof($earlybirdPromotions)>0){
                foreach($earlybirdPromotions as $earlybirdPromotion){

                    $selected_room_rate_plan = $earlybirdPromotion->selected_room_rate_plan;
                    $selected_room_rate_plan = json_decode($selected_room_rate_plan);
                    ...
                    foreach ($selected_room_rate_plan as $room) {
                        if ($room->room_type_id == $room_type_id) {
                            if (in_array($rate_plan_id, $room->selected_rate_plans)) {
                                ...
                                $applicable_room_and_rate_plan []= array($room_type_id => $rate_plan_id);
                                $max_discount_earlybird_promotions []= $earlybirdPromotion->discount_percentage;
                            }
                        }
                    }

                    $advance_booking_days = $earlybirdPromotion->advance_booking_days;
                    $stay_start_date = $earlybirdPromotion->stay_start_date;
                    $booking_start_date = $earlybirdPromotion->booking_start_date;
                    $booking_end_date = $earlybirdPromotion->booking_end_date;
        
                    $min_booking_date = date('Y-m-d', strtotime($stay_start_date . " - $advance_booking_days days"));

                    // dd($booking_date, $min_booking_date, $booking_start_date, $booking_end_date);
        
                    if ($booking_date >= $min_booking_date && $booking_date >= $booking_start_date && $booking_date <= $booking_end_date) {
                        $earlybirdRoomRatePlan = json_decode($earlybirdPromotion->selected_room_rate_plan, true);
                        ...
                        foreach ($earlybirdRoomRatePlan as $earlybirdPlan) {
                            if ($room_type_id == $earlybirdPlan['room_type_id'] && in_array($rate_plan_id, $earlybirdPlan['selected_rate_plans'])) {
                                if ($earlybirdPromotion->offer_type == 0) {
                                    $earlybirdPromotionDiscount = $earlybirdPromotion->discount_percentage;
                                } else {
                                    $earlybirdPromotionDiscount = $earlybirdPromotion->discounted_amount;
                                }
                            }
                        }
                    }
            ...
            if ($lastminutePromotions) {
                foreach($lastminutePromotions as $lastminutePromotion){

                    $stay_start_date = $lastminutePromotion->stay_start_date;
                    $booking_start_date = $lastminutePromotion->booking_start_date;
                    $booking_end_date = $lastminutePromotion->booking_end_date;
                    $booking_days_within = $lastminutePromotion->booking_days_within;

                    $checkInTimestamp = strtotime($date_from);
                    $bookingTimestamp = strtotime($booking_date);

                    $min_booking_date = date('Y-m-d', strtotime($stay_start_date . " - $booking_days_within days"));

                    // dd($booking_date, $min_booking_date, $date_from, $booking_start_date, $booking_end_date);

                    if ($booking_date <= $min_booking_date && $booking_date <= $date_from  && $booking_date >= $booking_start_date && $booking_date <= $booking_end_date) {
                        $lastminuteRoomRatePlan = json_decode($lastminutePromotion->selected_room_rate_plan, true);
                        ...
                        foreach ($lastminuteRoomRatePlan as $lastminutePlan) {
                            if ($room_type_id == $lastminutePlan['room_type_id'] && in_array($rate_plan_id, $lastminutePlan['selected_rate_plans'])) {
                                if ($lastminutePromotion->offer_type == 0) {
                                    $lastminutePromotionDiscount = $lastminutePromotion->discount_percentage;
                                } else {
                                    $lastminutePromotionDiscount = $lastminutePromotion->discounted_amount;
                                }
                            }
                        }
                    }
