...
use App\Http\Controllers\crs\CrsReservationController;
...
public function newReservation(Request $request)
{
  ...
  //$validCart=$validCart && $this->checkAgentCredit($for_user_id,$agent_credit,$hotel_id);
   if($validCart)
   {
        $inv_data=array();
       ...
       // $adjusted_amount=0;//For agent adjusted amount is 0
        ...
        $crs_data['invoice'] = $this->crsReservationService->createInvoiceForAgent($hotel_id,$cart,
        $from_date,$to_date,$inv_data['user_id'],$percentage);
                ...
                if($crs_reservation->fill($crs_data)->save())
                {
                    if($pay_type=="now")
                    {
                        ...
                        // {
                            ...
                            $res=array("status"=>1,"message"=>"Invoice details saved successfully","pay_type"=>$pay_type,"invoice_id"=>$cur_invoice->invoice_id,'invoice_secureHash'=>$invoice_secureHash);
                    ...
                    elseif($pay_type=='pay_later')
                    {
                        ...
                        if($invoice_details->save())
                        {
                            ...
                            if($this->crsReservationService->invoiceMail($cur_invoice->invoice_id,'Agent'))
                            {
                    ...
                    else if($pay_type=='both')
                    {
                        ...
                        //$this->crsReservationService->invoiceMail($cur_invoice->invoice_id,'Agent');
                        if($this->crsReservationService->preinvoiceMail($cur_invoice->invoice_id))
                        {
 ...
 *
 */
    public function checkAgentCredit($agent_id,$credit_amount,$hotel_id)
    {
        $agentCredit=0;
        $agentCredit=$this->calculateAgentCredit($agent_id,$hotel_id);
 ...
 *
 */
    public function calculateAgentCredit($agent_id,$hotel_id)
    {
        $agentCredit=0;
        $agentData = AdminUser::where('admin_id', $agent_id)
        ->first();
        $agentHotelData = AdminUser::where('username', $agentData['username'])
        ->where('hotel_id', $hotel_id)//Agent role
        ->first();
        $agentCreditModel=new AgentCredit();
        $agentCredit=$agentCreditModel->where('agent_id',$agentHotelData['admin_id'])->first();
        ...
        ->where('pay_status', '=', 1)
        ->where('for_user_id', '=',$agentHotelData['admin_id'])
