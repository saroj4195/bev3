...
?>
        ...
        <?php
            ...
            $checkout_session = \Stripe\Checkout\Session::create([
                ...
                'mode' => 'payment',
                'success_url' => 'https://be.bookingjini.com/success/' . $checksum . '/' . $booking_id . '/' . $booking_date . '/' . $amount,
                'cancel_url' => 'https://be.bookingjini.com/cancel',
            ]);
        ...
        if ($ap_SecureHash == $merchant_secure_hash) {
            if ($TRANSACTIONSTATUS == 200) {
                ...
                if ($url != '') {
                    ...
                    // $byke_group_hotel_ids =  [3033, 3035, 3036, 3037, 3038, 3039, 3043, 3045, 3046, 3047, 3053, 3057, 3058, 3419, 4479];
                    
                    // $fetch_hotel_id = DB::table('invoice_table')->where('invoice_id', $id)->select('hotel_id')->first();

                    // if (isset($fetch_hotel_id->hotel_id)) {
                    //     if (in_array($fetch_hotel_id->hotel_id, $byke_group_hotel_ids)) {
                    //         $url = 'https://www.thebyke.com/thank-you';
                    //     } else {
                    //         $url = $this->findURL($url, $id, $txnid);
                    //     }
                    // } else {
...
<?php
    ...
    {
        $hotel_info = DayOutingBookings::leftjoin('kernel.user_table', 'user_table.user_id', '=', 'day_outing_booking.user_id')
        ->leftjoin('kernel.hotels_table','hotels_table.hotel_id','=','day_outing_booking.hotel_id')
        ->leftjoin('kernel.company_table','company_table.company_id','=','hotels_table.company_id')
        ->where('day_outing_booking.booking_id', $invoice_id)
        ->select('day_outing_booking.hotel_id', 'hotels_table.hotel_name', 'day_outing_booking.paid_amount', 'user_table.first_name', 'user_table.last_name', 'user_table.email_id', 'user_table.mobile', 'user_table.address','company_table.company_url','hotels_table.company_id')->first();
