...
class BookingsController extends Controller
{
    ...
    //================================================================================================================================
    public function pilgrimageBookings($duration)
    {
        $today_date = date('Y-m-d');
        //$today_date = '2021-03-06';
        ...
        $all_hotels = CompanyDetails::
            join('hotels_table', 'hotels_table.company_id', '=', 'company_table.company_id')
            ->where('hotels_table.status', 1)
            ->where('hotels_table.city_id',1183)
            ->select('hotels_table.hotel_id', 'hotels_table.company_id')
            ->get();
    ...
    //================================================================================================================================
    public function getHotelBookingsData($hotel_id, $start_date, $end_date)
    {
        $summary = [];
        $day_wise_data = [];
        $msg = array('status'=>1,'summary'=>$summary,'details'=>$day_wise_data);
        return response()->json($msg);
    }
    //=============================================================================================
    public function getOccupancyOfRMHotelsOld(Request $request)
    {
        $status = 1;
        $message = 'Data found';
        $data = [];
        $from_date = date('Y-m-d');
        $from_date1 = date('Y-m-d',strtotime('+1 day'));
        $to_date = date('Y-m-d',strtotime('+6 days'));
        $str_ota_sql = '';
        $str_be_sql = '';
        $day_sl_no = 1;
        for($dt=$from_date;$dt<=$to_date;$dt++)
        {
            $d = date('Y-m-d',strtotime($dt));
            $str_ota_sql .= ",case when '$d' >= checkin_at and '$d' < checkout_at  then rooms_qty else 0 end as booked_rooms_day$day_sl_no";
            $str_be_sql .= ",case when '$d' >= check_in and '$d' < check_out  then rooms else 0 end as booked_rooms_day$day_sl_no";
            $day_sl_no++;
        }
        $str_ota_sql = substr($str_ota_sql,1);
        $str_be_sql = substr($str_be_sql,1);
        //get RM hotels
        $hotel_sql = "
            SELECT T.hotel_id, T.hotel_name, sum(T.total_rooms) AS total_rooms
            FROM (
            ...
            from kernel.hotels_table A
            inner join kernel.room_type_table B ON A.hotel_id = B.hotel_id
            inner join kernel.billing_table C ON A.company_id = C.company_id
            where C.product_name LIKE '%REVMGMT%'
            order by A.hotel_name, B.room_type_id) T
            group by T.hotel_id, T.hotel_name
            order by T.hotel_name
        ";
        $rm_hotels = DB::select("$hotel_sql");

        foreach($rm_hotels as $rm_hotel)
        {
            $record['hotel_id'] = $rm_hotel->hotel_id;
            $record['hotel_name'] = $rm_hotel->hotel_name.' ('.$rm_hotel->hotel_id.') Rooms: '.$rm_hotel->total_rooms;
            $arr_occupancy = [];
            for($dt=$from_date;$dt<=$to_date;$dt++)
            {
                $occupancy_record['date'] = date('d-m-Y',strtotime($dt));
                $occupancy_record['dt'] = date('Y-m-d',strtotime($dt));
                $occupancy_record['booked_rooms'] = 0;
                $occupancy_record['occupancy_percentage'] = 0;
                $arr_occupancy[] = $occupancy_record;
            }
            $record['occupancy'] = $arr_occupancy;
            $data[] = $record;
        }
        
        
        $ota_sql = "select T.hotel_id, sum(booked_rooms_day1) AS b1, sum(booked_rooms_day2) AS b2, sum(booked_rooms_day3) AS b3, 
        sum(booked_rooms_day4) AS b4, sum(booked_rooms_day5) AS b5, sum(booked_rooms_day6) AS b6, sum(booked_rooms_day7) AS b7
        from 
        (select A.hotel_id, $str_ota_sql
            ...
            and (A.checkin_at between '$from_date' and '$to_date' or A.checkout_at between '$from_date1' and '$to_date')
            and A.confirm_status = 1
            and A.cancel_status = 0)T
            group by T.hotel_id";
        $ota_bookings = DB::connection('bookingjini_cm')->select("$ota_sql");
        ...
        foreach($ota_bookings as $ota_booking)
        {
            ...
            {
                if($record['hotel_id'] == $ota_booking->hotel_id)
                {
                    $data[$idx]['occupancy'][0]['booked_rooms'] = $ota_booking->b1;
                    $data[$idx]['occupancy'][1]['booked_rooms'] = $ota_booking->b2;
                    $data[$idx]['occupancy'][2]['booked_rooms'] = $ota_booking->b3;
                    $data[$idx]['occupancy'][3]['booked_rooms'] = $ota_booking->b4;
                    $data[$idx]['occupancy'][4]['booked_rooms'] = $ota_booking->b5;
                    $data[$idx]['occupancy'][5]['booked_rooms'] = $ota_booking->b6;
                    $data[$idx]['occupancy'][6]['booked_rooms'] = $ota_booking->b7;
                }
                $idx++;
            }
        }
        ...
        $be_sql = "select T.hotel_id, sum(booked_rooms_day1) AS b1, sum(booked_rooms_day2) AS b2, sum(booked_rooms_day3) AS b3, 
        sum(booked_rooms_day4) AS b4, sum(booked_rooms_day5) AS b5, sum(booked_rooms_day6) AS b6, sum(booked_rooms_day7) AS b7
        from 
        (select A.hotel_id, $str_be_sql
        
            from
            booking_engine.hotel_booking A
            where A.hotel_id in (
            select A.hotel_id
            from kernel.hotels_table A
            inner join kernel.room_type_table B ON A.hotel_id = B.hotel_id
            ...
            and (A.check_in between '$from_date' and '$to_date' or A.check_out between '$from_date1' and '$to_date')
            and A.booking_status = 1)T
            group by T.hotel_id";
        $be_bookings = DB::select("$be_sql");
        ...
        foreach($be_bookings as $be_booking)
        {
            ...
            foreach($data as $record)
            {
                if($record['hotel_id'] == $be_booking->hotel_id)
                {
                    $data[$idx]['occupancy'][0]['booked_rooms'] += $be_booking->b1;
                    $data[$idx]['occupancy'][1]['booked_rooms'] += $be_booking->b2;
                    $data[$idx]['occupancy'][2]['booked_rooms'] += $be_booking->b3;
                    $data[$idx]['occupancy'][3]['booked_rooms'] += $be_booking->b4;
                    $data[$idx]['occupancy'][4]['booked_rooms'] += $be_booking->b5;
                    $data[$idx]['occupancy'][5]['booked_rooms'] += $be_booking->b6;
                    $data[$idx]['occupancy'][6]['booked_rooms'] += $be_booking->b7;
                }
                $idx++;
            }
        }
        ...
        foreach($rm_hotels as $rm_hotel)
        {
            ...
            foreach($data as $record)
            {
                if($record['hotel_id'] == $rm_hotel->hotel_id)
                {
                    $data[$idx]['occupancy'][0]['occupancy_percentage'] = round(($data[$idx]['occupancy'][0]['booked_rooms']/$rm_hotel->total_rooms)*100,2);
                    $data[$idx]['occupancy'][1]['occupancy_percentage'] = round(($data[$idx]['occupancy'][1]['booked_rooms']/$rm_hotel->total_rooms)*100,2);;
                    $data[$idx]['occupancy'][2]['occupancy_percentage'] = round(($data[$idx]['occupancy'][2]['booked_rooms']/$rm_hotel->total_rooms)*100,2);;
                    $data[$idx]['occupancy'][3]['occupancy_percentage'] = round(($data[$idx]['occupancy'][3]['booked_rooms']/$rm_hotel->total_rooms)*100,2);;
                    $data[$idx]['occupancy'][4]['occupancy_percentage'] = round(($data[$idx]['occupancy'][4]['booked_rooms']/$rm_hotel->total_rooms)*100,2);;
                    $data[$idx]['occupancy'][5]['occupancy_percentage'] = round(($data[$idx]['occupancy'][5]['booked_rooms']/$rm_hotel->total_rooms)*100,2);;
                    $data[$idx]['occupancy'][6]['occupancy_percentage'] = round(($data[$idx]['occupancy'][6]['booked_rooms']/$rm_hotel->total_rooms)*100,2);;
                }
                $idx++;
            }
        }
    ...
    //=============================================================================================
    public function getOccupancyOfRMHotels(Request $request)
    {
        $status = 1;
        $message = 'Data found';
        $data = [];
        ...
        $ota_sql = "select hotel_id, stay_day, sum(available) as available
            from (
            select stay_day, hotel_id, room_type_id, avg(no_of_rooms)  as available
            from dp_cur_inv_table
            where stay_day between '$from_date' and '$to_date'
            group by stay_day, hotel_id, room_type_id) T
            group by T.stay_day, T.hotel_id
            order by hotel_id, stay_day
        ";
        $room_availability = [];
        $ota_availability = DB::connection('bookingjini_cm')->select("$ota_sql");
        foreach($ota_availability as $ota_available)
        {
            $room_availability[$ota_available->hotel_id][$ota_available->stay_day] = $ota_available->available;
        }
        ...
        foreach($rm_hotels as $rm_hotel)
        {
            ...
            for($i=0;$i<7;$i++)
            {
                $date = date('d-m-Y',strtotime($from_date."+$i day"));
                $dt = date('Y-m-d',strtotime($from_date."+$i day"));
                $occupancy_record['date'] = $date;
                $occupancy_record['dt'] = $dt;
                $available_rooms = isset($room_availability[$rm_hotel->hotel_id][$dt]) ? $room_availability[$rm_hotel->hotel_id][$dt] : 0;
                $booked_rooms = $rm_hotel->total_rooms - $available_rooms;
                $occupancy_record['available_rooms'] = $available_rooms;
                $occupancy_record['booked_rooms'] = $booked_rooms;
                $occupancy_record['occupancy_percentage'] = round($booked_rooms/$rm_hotel->total_rooms *100,2);
                $arr_occupancy[] = $occupancy_record;
            }
            $record['occupancy'] = $arr_occupancy;
            $data[] = $record;
    ...
    //=============================================================================================
    public function getUnpaidBookingList(Request $request)
    {
        ...
        if($hotel_id == "")
        {
            $invoices = Invoice::join('kernel.user_table as user_table', 'invoice_table.user_id', '=', 'user_table.user_id')
                ->join('kernel.hotels_table as hotels_table', 'invoice_table.hotel_id', '=', 'hotels_table.hotel_id')
                ->join('kernel.company_table as company_table', 'company_table.company_id', '=', 'hotels_table.company_id')
                ->join('hotel_booking', 'invoice_table.invoice_id', '=', 'hotel_booking.invoice_id')
                ->select('invoice_table.invoice_id', 'invoice_table.hotel_id',
                'invoice_table.hotel_name', 'invoice_table.room_type', 
                'invoice_table.total_amount', 'invoice_table.paid_amount', 
                'invoice_table.user_id', 'invoice_table.paid_amount', 
                'invoice_table.check_in_out', 'invoice_table.booking_date', 
                'invoice_table.booking_status', 'user_table.first_name', 
                'user_table.last_name', 'user_table.email_id', 'user_table.address', 
                'user_table.mobile', 'hotel_booking.rooms', 
                'hotel_booking.check_in', 'hotel_booking.check_out')
                ...
                ->whereBetween('invoice_table.booking_date',[$from_date,$to_date])
                ->distinct('invoice_id')
        ...
        {
            $invoices = Invoice::join('kernel.user_table as user_table', 'user_table.user_id', '=', 'invoice_table.user_id')
                ->join('hotel_booking', 'invoice_table.invoice_id', '=', 'hotel_booking.invoice_id')
                ->select('invoice_table.invoice_id', 'invoice_table.hotel_id',
                'invoice_table.hotel_name', 'invoice_table.room_type', 
                'invoice_table.total_amount', 'invoice_table.paid_amount', 
        ...
        foreach ($invoices as $invoice) 
        {
            ...
            $booking_id = date('dmy',strtotime($invoice->booking_date)).$invoice->invoice_id;
            $guest_name = $invoice->first_name . ' ' . $invoice->last_name;
            $unpaid_booking['hotel_id'] = $invoice->hotel_id;
            $unpaid_booking['hotel_name'] = $invoice->hotel_name;
            $unpaid_booking['booking_id'] = $booking_id;
            $unpaid_booking['booking_date'] = $invoice->booking_date;
            $unpaid_booking['guest_name'] = $guest_name;
            $unpaid_booking['guest_mobile'] = $invoice->mobile;
            $unpaid_booking['guest_email'] = $invoice->email_id;
            $ckinout = str_replace("[","",$invoice->check_in_out);
            $ckinout = str_replace("]","",$ckinout);
            $x = explode("-",$ckinout);
            $check_in = "$x[2]-$x[1]-$x[0]";
            $check_out = "$x[5]-$x[4]-$x[3]";
            $unpaid_booking['check_in'] = $check_in;
            $unpaid_booking['check_out'] = $check_out;
            $arr_room_types = json_decode($invoice->room_type);
            $str_room_types = implode(", ",$arr_room_types);
            $unpaid_booking['room_type'] = $str_room_types;
            $unpaid_booking['booking_amount'] = $invoice->total_amount;
            $unpaid_bookings[] = $unpaid_booking;
        }
        $format = [
            ["name" => "Hotel ID", "field" => "hotel_id", "options" => ["filter" => true, "sort" => true, "headerNoWrap"=>true]], 
            ["name" => "Hotel Name", "field" => "hotel_name", "options" => ["filter" => true, "sort" => true, "headerNoWrap"=>true]], 
            ["name" => "Booking ID", "field" => "booking_id", "options" => ["filter" => true, "sort" => true, "headerNoWrap"=>true]],
            ["name" => "Booking Date", "field" => "booking_date", "options" => ["filter" => true, "sort" => true, "headerNoWrap"=>true]], 
            ["name" => "Guest Name", "field" => "guest_name", "options" => ["filter" => true, "sort" => true, "headerNoWrap"=>true]], 
            ["name" => "Guest Email", "field" => "guest_email", "options" => ["filter" => true, "sort" => true, "headerNoWrap"=>true]], 
            ["name" => "Guest Phone", "field" => "guest_mobile", "options" => ["filter" => true, "sort" => true, "headerNoWrap"=>true]],
            ["name" => "Room Type", "field" => "room_type", "options" => ["filter" => true, "sort" => true, "headerNoWrap"=>true]],
            ["name" => "Check in", "field" => "check_in", "options" => ["filter" => true, "sort" => true, "headerNoWrap"=>true]], 
            ["name" => "Check out", "field" => "check_out", "options" => ["filter" => true, "sort" => true, "headerNoWrap"=>true]], 
            ["name" => "Total Amount", "field" => "booking_amount", "options" => ["filter" => true, "sort" => true, "headerNoWrap"=>true]]
            
        ];
        if($unpaid_bookings != [])
        {
            $msg = array('status' => 1,'message'=>'Bookings','unpaid_bookings' => $unpaid_bookings, 'format' => $format);
            return response()->json($msg);
        }
        else
        {
            $msg = array('status' => 0,'message'=>'No Bookings Found');
            return response()->json($msg);
        } 
