...
class PublicUserController extends Controller
{
    ...
    **/
    public function register(Request $request)
    {
        ...
        $data=$request->all();
        //Get company id
        $data['company_url'] = trim($data['company_url']);
        $conditions=array('company_url'=>$data['company_url']);
        $company_details=DB::table('kernel.company_table')->where($conditions)->first();

        if(empty($company_details)){
            $data['company_url'] = 'www.'. $data['company_url'];
            $company_details=DB::table('kernel.company_table')->where('company_url',$data['company_url'])->first();
        }
    ...
    **/
    ...
    public function upcomingBookings(Request $request)
    {
        ...
        $today = date('Y-m-d');
        $details = Invoice::join('kernel.user_table','invoice_table.user_id','=','user_table.user_id')
        ->join('kernel.hotels_table','hotels_table.hotel_id','=','invoice_table.hotel_id')
        ->join('kernel.city_table','hotels_table.city_id','=','city_table.city_id')
        ->join('kernel.state_table','hotels_table.state_id','=','state_table.state_id')
        ->select(
            'invoice_table.hotel_id', 'hotels_table.hotel_name', 'city_table.city_name', 'state_table.state_name',
            'invoice_table.booking_status','invoice_table.booking_date', 'invoice_table.invoice_id',
            'invoice_table.check_in_out',
            'user_table.first_name','user_table.last_name','user_table.email_id',
            'user_table.address','user_table.mobile',
            DB::raw("REPLACE(SUBSTRING_INDEX(invoice_table.check_in_out,'-',3),'[','') AS check_in"),
            DB::raw("REPLACE(SUBSTRING_INDEX(invoice_table.check_in_out,'-',-3),']','') AS check_out")
        )
        ->where([
            ['invoice_table.user_id',$user->user_id],
            ['invoice_table.booking_status',1],
            [DB::raw("REPLACE(SUBSTRING_INDEX(invoice_table.check_in_out,'-',3),'[','')"),'>=',$today]
            ]
        )
        ->orderBy(DB::raw("REPLACE(SUBSTRING_INDEX(invoice_table.check_in_out,'-',3),'[','')"),'ASC')
        ->get();  
   
        foreach($details as $data_up)
        {
            $hotel_id = $data_up->hotel_id;
            $booking_id     = date("dmy", strtotime($data_up->booking_date)).str_pad($data_up->invoice_id, 4, '0', STR_PAD_LEFT);
            $data_up->booking_id =  $booking_id;
            // $x = str_replace('[','',$data_up->check_in_out);
            // $x = str_replace(']','',$x);
            // $x = explode('-',$x);
            // $d1 = $x[0].'-'.$x[1].'-'.$x[2];
            // $d2 = $x[3].'-'.$x[4].'-'.$x[5];
            $data_up->check_in = date('M d Y',strtotime($data_up->check_in));
            $data_up->check_out = date('M d Y',strtotime($data_up->check_out));
            $data_up->hotel_address =  $data_up->city_name.', '.$data_up->state_name;
            $dt1 = date_create($data_up->check_in);
            $dt2 = date_create($data_up->check_out);
            $data_up->no_of_nights = date_diff($dt1,$dt2)->format('%a');
        }
        if(sizeof($details)>0)
        {
            $res = array('status'=>1,'message'=>'details retrive sucessfully','bookings'=>$details);
            return response()->json($res);
        }
        else
        {
            $res = array('status'=>0,'message'=>'No upcoming Bookings');
            return response()->json($res);
        }
    }
    //================================================================================================================================
    public function cancelledBookings(Request $request)
    {
        ...
        ->orderBy(DB::raw("SUBSTR(SUBSTRING_INDEX(invoice_table.check_in_out,'-',3),2)"),'ASC')
        ->get();  
        ...
        {
            $res = array('status'=>0,'message'=>'No cancelled bookings');
            return response()->json($res);
        }
    }
    //================================================================================================================================
    public function completedBookings(Request $request)
    {
        ...
        {
            $res = array('status'=>0,'message'=>'No completed Bookings');
            return response()->json($res);
        }
    }
    //============================================================================================================================
    public function getBEBookingDetails(Request $request)
    {
        $can_modify = FALSE;
        $can_cancel = FALSE;
        $today = date('Y-m-d');
        ...
        $booking_details = Invoice::join('kernel.user_table','invoice_table.user_id','=','user_table.user_id')
        ->select(
        'invoice_table.hotel_name','invoice_table.total_amount','invoice_table.paid_amount',
        'invoice_table.hotel_id','invoice_table.invoice_id','invoice_table.user_id',
        'invoice_table.booking_status','invoice_table.booking_date','invoice_table.room_type',
        'invoice_table.check_in_out','invoice_table.extra_details','user_table.first_name',
        'user_table.last_name','user_table.email_id','user_table.address','user_table.mobile',
        'invoice_table.package_id', 'invoice_table.room_type',
        DB::raw("REPLACE(SUBSTRING_INDEX(invoice_table.check_in_out,'-',3),'[','') AS check_in"),
        DB::raw("REPLACE(SUBSTRING_INDEX(invoice_table.check_in_out,'-',-3),']','') AS check_out"),
        'invoice_table.paid_service_id')
        ->where([
            ['invoice_table.user_id',$user->user_id],
            ['invoice_table.invoice_id',$invoice_id]
            ]
        )
        ->first();  

        $data_up = $booking_details;
        $adult = array();
        $child = array(); 
        $hotel_id = $data_up->hotel_id;
        $data_up->send_invoice_id = $data_up->invoice_id;
        ...
        //hotel information
        $hotel_details = HotelInformation::where('hotel_id',$hotel_id)
        ->select('exterior_image','city_table.city_name','state_table.state_name','hotels_table.hotel_address')
        ->join('kernel.city_table','hotels_table.city_id','=','city_table.city_id')
        ->join('kernel.state_table','hotels_table.state_id','=','state_table.state_id')
        ->first();

        $arr_hotel_images = explode(',',$hotel_details->exterior_image);
        $hotel_image_id = $arr_hotel_images[0];
        $hotel_image_details = ImageTable::where('image_id',$hotel_image_id)->first();
        $hotel_image_url = env('S3_IMAGE_PATH').$hotel_image_details->image_name;
        $data_up->hotel_image = $hotel_image_url;
        $data_up->hotel_address = $hotel_details->hotel_address;
        $data_up->hotel_location = $hotel_details->city_name .', '.$hotel_details->state_name;
        ...
        //room types
        $room_type_details = Invoice::join('kernel.user_table','invoice_table.user_id','=','user_table.user_id')
        ->join('hotel_booking','invoice_table.invoice_id','hotel_booking.invoice_id')
        ->join('kernel.room_type_table','hotel_booking.room_type_id','room_type_table.room_type_id')
        ->select('hotel_booking.room_type_id','rooms','room_type_table.room_type AS room_type_name','room_type_table.image'
        )
        ->where([
            ['invoice_table.user_id',$user->user_id],
            ['invoice_table.invoice_id',$invoice_id]
        ...
        ->get();  
        $arr_room_types = array();
        foreach($room_type_details as $room_type_detail)
        {
            
            $arr_room_type_images = explode(',',$room_type_detail->image);
            $room_type_image_id = $arr_room_type_images[0];
            $room_type_image_details = ImageTable::where('image_id',$room_type_image_id)->first();
            $room_type_image_url = env('S3_IMAGE_PATH').$room_type_image_details->image_name;

            $room_type_record['room_type_id'] = $room_type_detail['room_type_id'];
            $room_type_record['no_of_rooms'] = $room_type_detail['rooms'];
            $room_type_record['room_type_name'] = $room_type_detail['room_type_name'];
            $room_type_record['image'] = $room_type_image_url;
            $room_type_record['meal_plan'] = 'EP';
            $arr_room_types[] = $room_type_record;
        }
        $data_up->room_types = $arr_room_types;
        ...
        $data_up->total_paid_service_amount = $total_paid_service_amount;
        if($data_up->booking_status == 1)
        {
            if($data_up->check_in > $today)
            {
                $can_modify = TRUE;
                $can_cancel = TRUE;
            }
        ...
        $data_up->can_cancel = $can_cancel;
        $data_up->can_modify = $can_modify;
        if($data_up)
        {
            $res=array('status'=>1,'message'=>'details retrive sucessfully','booking_data'=>$data_up);
            return response()->json($res);
        }
        else
        {
            $res=array('status'=>0,'message'=>'booking details not found');
            return response()->json($res);
        }
    }
    //================================================================================================================================
    
    //@author saroj patel dt-28-01-23 
    //this API is use in mobile
    public function fetchGuestBookings(Request $request,$type){
        $data = $request->all();
        // $hotel_id = $data['hotel_id'];
        $date=date('Y-m-d',strtotime($data['date']));
        $last_date=date('Y-m-t',strtotime($data['date'].'+30 days'));
        ...
        if($type == 'confirm'){
            $types = ['invoice_table.booking_status',1];
        }else if($type == 'completed'){
            $types = ['invoice_table.booking_status',1];
        }else{  
            $types = ['invoice_table.booking_status',3];
        }
        
        $failour_message="details retrive fails";
       
        $user=User::select('user_id')->where('mobile',$mobile)->where('company_id',$data['company_id'])->orderBy('user_id','DESC')->first();


        $details=Invoice::join('kernel.user_table','invoice_table.user_id','=','user_table.user_id')
        ->join('hotel_booking','invoice_table.invoice_id','=','hotel_booking.invoice_id')
        ->join('kernel.hotels_table','invoice_table.hotel_id','=','hotels_table.hotel_id')
        ->where('kernel.hotels_table.company_id',$data['company_id'])
        ->select(DB::raw("group_concat(hotel_booking.room_type_id) as room_type_id"),DB::raw("group_concat(hotel_booking.rooms) as rooms"),
        'invoice_table.hotel_name','invoice_table.total_amount','invoice_table.paid_amount','invoice_table.hotel_id','invoice_table.invoice_id','invoice_table.user_id','invoice_table.booking_status','invoice_table.booking_date','invoice_table.room_type','invoice_table.check_in_out','invoice_table.extra_details','user_table.first_name','user_table.last_name','user_table.email_id','user_table.address','user_table.mobile','hotel_booking.check_in','hotel_booking.check_out','invoice_table.package_id','hotels_table.hotel_address')->where([['invoice_table.user_id',$user->user_id],$types,['hotel_booking.check_in','>=',$date],['hotel_booking.check_in','<=',$last_date]]) ->groupBy('invoice_table.invoice_id')->orderBy('invoice_table.invoice_id','DESC')->paginate(12);  
        ...
        foreach($details as $data_up)
        {
            ...
            $booking_data["room_type_id"] = $data_up->room_type_id;
            $booking_data["rooms"] = $data_up->rooms;
            $booking_data["hotel_name"] = $data_up->hotel_name;
            $booking_data["paid_amount"] = $data_up->paid_amount;
            $booking_data["hotel_id"] = $data_up->hotel_id;
            $booking_data["invoice_id"] = $data_up->invoice_id;
            $booking_data["user_id"] = $data_up->user_id;
            $booking_data["booking_status"] = $data_up->booking_status;
            $booking_data["booking_date"] = $data_up->booking_date;
            $booking_data["room_type"] = $data_up->room_type;
            $booking_data["check_in_out"] = $data_up->check_in_out;
            $booking_data["extra_details"] = $data_up->extra_details;
            $booking_data["first_name"] = $data_up->first_name;
            $booking_data["last_name"] = $data_up->last_name;
            $booking_data["email_id"] = $data_up->email_id;
            $booking_data["address"] = $data_up->address;
            $booking_data["mobile"] = $data_up->mobile;
            $booking_data["check_in"] = $data_up->check_in;
            $booking_data["check_out"] = $data_up->check_out;
            $booking_data["package_id"] = $data_up->package_id;
            $booking_data["hotel_address"] = $data_up->hotel_address;
            $booking_data["send_invoice_id"] = $data_up->send_invoice_id;
            $booking_data["display_room_type"] = $room_dlt;
            $booking_data["adult"] = $adult;
            $booking_data["child"] = $child;
       
            if($type == 'completed'){
                
                if($data_up->check_out < $today){
        ...
        {
            $res=array('status'=>1,'message'=>'details retrive sucessfully','details'=>$all_booking_data);
            return response()->json($res);
        }
        else{
            $res=array('status'=>0,'message'=>'details retrive fails');
