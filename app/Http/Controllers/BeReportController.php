...
class BeReportController extends Controller
{
  ...
  public function noOfBEBookingsBetweenDays($hotel_id, $from_date, $to_date)
  {
  ...
  public function noOfBEBookingsBetweenDates($hotel_id, $from_date, $to_date, $date_type)
  {
    $from_date = date('Y-m-d',strtotime($from_date));
    $to_date = date('Y-m-d',strtotime($to_date));
    if($date_type == 'BOOKING')
    {
      $beBookings = DB::select(DB::raw("SELECT count(invoice_table.invoice_id) as noOfBooking, 
        SUM(CAST(`total_amount` AS DECIMAL(10,2))) as totalAmount 
        FROM invoice_table 
        WHERE invoice_table.booking_status = 1 
        AND date(booking_date) between '$from_date' AND '$to_date' 
        AND hotel_id = '$hotel_id' 
        AND booking_source IN ('WEBSITE','QUICKPAYMENT')
        GROUP BY invoice_table.hotel_id"));
    }
    else if($date_type == 'CHECKIN')
    {
      $beBookings = DB::select(DB::raw("SELECT count(invoice_table.invoice_id) as noOfBooking, 
      SUM(CAST(`total_amount` AS DECIMAL(10,2))) as totalAmount 
      FROM invoice_table 
      WHERE invoice_table.booking_status = 1 
      AND SUBSTRING_INDEX(REPLACE(left(invoice_table.check_in_out,11),'[',''),'-',3) between '$from_date' AND '$to_date' 
      AND hotel_id = '$hotel_id' 
      GROUP BY invoice_table.hotel_id"));
    }
    if($beBookings)
