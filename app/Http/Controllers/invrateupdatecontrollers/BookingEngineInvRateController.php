  ...
  private $blk_messages = [
    ...
    public function singleRateUpdate(Request $request){
          ...
          if($otaid == -1){
              ...
              foreach($rates_data as $rates)
              {
                  foreach($rates['rates'] as $rate)
                  {
                      ...
                      $get_los = Inventory::select('los')->where('hotel_id',$imp_data['hotel_id'])
                                                        ->where('room_type_id',$rates['room_type_id'])
                                                        ->where('date_from','<=',$rate['date'])
                                                        ->where('date_to','>=',$rate['date'])
                                                        ->orderBy('inventory_id','DESC')
                                                        ->first();
                      $los_data = isset($get_los->los)?$get_los->los:1;
                      $ratedata['los']=$los_data;//Default length of stay
                      ...
                      $max_price = (int)$price->max_price;
                      if($rate['bar_price'] >= $min_price && $rate['bar_price'] < $max_price)
                      {
                          $bp=1;
                      }
                      if($bp==0)
                      {
                          $res=array('status'=>0,'message'=>"price should be equal or greater than: ".$min_price." and should be lessthan: ".$max_price);
                          return $res;
                      }
                      foreach($rate['multiple_occupancy'] as $multi){
                          $multi_occupancy = (int)$rate['multiple_occupancy'][0];
                          if($multi_occupancy >= $min_price && $multi_occupancy < $max_price){
                              $mp=1;
                          }
                      }
                      if($mp==0)
                      {
                          return array('status'=>0,'message'=>"price should be equal or greater than: ".$min_price." and should be lessthan: ".$max_price);
                      }
                      $success_flag=1;
                      try{
             ...
             if($otaid == -1) {
                 ...
                 if($rate_plan_log->fill($be_opt)->save())
                 {
                    ...
                    if($this->googleHotelStatus($data['hotel_id'])){
                        ...
                        if($be_opt['from_date'] == $be_opt['to_date']){
                            $to_date = date('Y-m-d',strtotime($be_opt['to_date'].'+1 day'));
                        }
                        else{
     ...
     public function rateUpdateToGoogleAds($hotel_id,$room_type_id,$rate_plan_id,$from_date,$to_date,$bar_price,$multiple_days){
        ...
        $get_occupancy = MasterRoomType::select('max_people')->where('hotel_id',$hotel_id)->where('room_type_id',$room_type_id)->first();
        $occupancy = isset($get_occupancy->max_people)?$get_occupancy->max_people:2;
