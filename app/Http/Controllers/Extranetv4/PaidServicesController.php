...
public function addNewPaidService(Request $request){
    ...
    $failure_message = FAILED_PAID_SERVICE_MESSAGE;
    ...
    if($paid_services->checkPaidServiceStatus($data['service_name'],$data['hotel_id'])=="new"){
        if($paid_services->fill($data)->save()){
            $res=array('status'=>1,"message"=>SAVED_PAID_SERVICE_MESSAGE);
        ...
        $res=array('status'=>1,"message"=>ALREADY_EXIST_PAID_SERVICE_MESSAGE);
        return response()->json($res);;
    }
}
    /**
...
public function updatePaidService(int $id,Request $request){
    ...
    if($paid_services->paid_service_id == $id){
        if($paid_services->fill($data)->save()){
            $res=array('status'=>1,"message"=>UPDATED_PAID_SERVICE_MESSAGE);
            return response()->json($res);
        }else{
            $res=array('status'=>-1,"message"=>$failure_message);
            $res['errors'][] = INTERNAL_SERVER_ERR_MESSAGE;
...
{
$res=array('status'=>1,"message"=>RETRIEVED_PAID_SERVICE_MESSAGE,'paidServices'=>$paidServices);
return response()->json($res);
}
else
{
$res=array('status'=>0,"message"=>NO_PAID_SERVICE_MESSAGE);
return response()->json($res);
}
}
else
...
if($paidService)
{
$res=array('status'=>1,"message"=>RETRIEVED_PAID_SERVICE_MESSAGE,'paidService'=>$paidService);
return response()->json($res);
}
else
{
...
{
                    $res=array('status'=>1,"message"=>DELETED_PAID_SERVICE_MESSAGE);
return response()->json($res);
            }
else
            {
