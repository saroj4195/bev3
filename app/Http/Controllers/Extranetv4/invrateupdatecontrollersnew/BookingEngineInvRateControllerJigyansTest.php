...
class BookingEngineInvRateControllerJigyansTest extends Controller
{
    ...
    public function roomTypeWiselosUpdateToGoogleAds($data,$hotel_id)
    {
        ...
        foreach($inv_start_date_range as $RsStartDate)
        {
            $rate_plan_datas[$counter] = $getRatePlans->where("room_type_id",$RsStartDate['room_type_id'])->toArray();
            foreach($rate_plan_datas[$counter] as $Rslosdetails)
            {
                $xml_data .=   '<AvailStatusMessage>
                <StatusApplicationControl Start="' . $RsStartDate['date'] . '"
    ...
    //Added by Jigyans dt : - 20-03-2023
    public function bulkBeOtaPushNew111111111($imp_data, $data)
    {
        $rate_plan_log = new RatePlanLog();
        $logModel      = new RateUpdateLog();
        $base_rate     = new BaseRate();
        ...
        if (is_int($rate_plan_log_ids)) {
            ...
            if ($this->googleHotelStatus($data['hotel_id'])) {
                ...
                if (isset($google_resp["Success"])) {
                    ...
                    curl_setopt($los_ch, CURLOPT_HTTPHEADER, $headers);
                    curl_setopt($los_ch, CURLOPT_RETURNTRANSFER, true);
                    curl_setopt($los_ch, CURLOPT_POSTFIELDS, $xml_data);
    ...
    //Modified by Jigyans dt : - 21-06-2023
        ...
        if (isset($get_rates_datas->status)) {
            if ($get_rates_datas->status == 1) {
                if (count($get_rates_datas->channel_rates) > 0) {
                    ...
                    {
                        // print_r($get_rates_datas[0]->plans);exit;
    ...
    public function bulkBeOtaPushNew3333($imp_data, $data)
    {
        $rate_plan_log = new RatePlanLog();
        $logModel      = new RateUpdateLog();
        $base_rate     = new BaseRate();
        ...
        while (strtotime($from_date) <= strtotime($to_date)) {
            ...
            $timestamp = strtotime($array);
            $day = date('D', $timestamp);
            array_push($all_dates_array, $array);
            $from_date = date("Y-m-d", strtotime("+1 days", strtotime($from_date)));
        }
        ...
        $unblocked_be_dates = array();
        $blocked_be_datas = array();
        $blockedbedates = array();
            ...
            if ($get_rates_datas->status == 1) {
                if (count($get_rates_datas->channel_rates) > 0) {
                    ...
                    if(count($get_rates_datas) > 0)
                    {
                        ...
                        $blocked_be_datas = array_filter($get_rates_datas, function ($item) {
                            return $item['block_status'] == 1;
                        });
    
                        $blockedbedates = array_column(array_values($blocked_be_datas), "rate_date");
    
                        $blocked_be_index_datas = array_keys($blocked_be_datas);
    
                        foreach ($blocked_be_index_datas as $BlockedBeDatas) {
                            unset($get_rates_datas[$BlockedBeDatas]);
                        }
    
                        $get_rates_datas = array_values($get_rates_datas);
                        $unblocked_be_dates = array_column($get_rates_datas, "rate_date");
    
                        $filtered_dates = $unblocked_be_dates;
                    }
                    else{
                        $filtered_dates = $all_dates_array;
                    }
        ...
        $blocked_rate_bar_price = collect($blocked_be_datas);
        foreach ($all_dates_array as $RsAllDates) {
            if (in_array($RsAllDates, $blockedbedates)) {
                $be_rate_datas[] = array(
                    "hotel_id" => $imp_data['hotel_id'],
                    "room_type_id" => $data['room_type_id'],
                    "rate_plan_id" => $data['rate_plan_id'],
                    "multiple_occupancy" => json_encode($blocked_rate_bar_price->where("rate_date", $RsAllDates)->first()['multiple_occupancy']),
                    "bar_price" => $blocked_rate_bar_price->where("rate_date", $RsAllDates)->first()['bar_price'],
                    "from_date" => $RsAllDates,
                    "to_date" => $RsAllDates,
                    "multiple_days" => $data['multiple_days'],
                    "block_status" => 1,
                    "los" => $blocked_rate_bar_price->where("rate_date", $RsAllDates)->first()['los'],
                    "client_ip" => $imp_data['client_ip'],
                    "user_id" => $imp_data['user_id'],
                    "extra_adult_price" => $blocked_rate_bar_price->where("rate_date", $RsAllDates)->first()['extra_adult_price'],
                    "extra_child_price" => $blocked_rate_bar_price->where("rate_date", $RsAllDates)->first()['extra_child_price']
                );
            } elseif (in_array($RsAllDates, $filtered_dates)) {
                $be_rate_datas[] = array(
                    "hotel_id" => $imp_data['hotel_id'],
                    "room_type_id" => $data['room_type_id'],
                    "rate_plan_id" => $data['rate_plan_id'],
        ...
        // $be_rate_datas = collect($be_rate_datas)->groupBy('block_status')->toArray();
        // print_r($be_rate_datas);exit;

        if (count($be_rate_datas) > 0) {
            $google_hotel_status = "";
            $rate_plan_log_ids = $rate_plan_log->insert($be_rate_datas);
            $index = "";
            if ($rate_plan_log_ids == 1) {
                if (!isset($imp_data['dp_status'])) {
                    foreach ($be_rate_datas as $RsRateDatas) {
                        $RsRateDatas['bar_price'] = $data['bar_price'];
                    }
                ...
                if ($this->googleHotelStatus($data['hotel_id'])) {
                    ...
                    if ((count($blocked_be_datas) > 0)) {
                        ...
                        foreach($all_dates_array as $Rsalldatesarray)
                        {
                            $current_rate_data[] = [
                                "hotel_id"          => $hotel_id,
                                "room_type_id"        => $room_type_id,
                        ...
                        $currentrates =  $current_rate_data;
                    ...
                    if (isset($google_resp["Success"])) {
                        ...
                        if (count($current_rates) > 0) {
                            ...
                            if (count($new_rate_datas) > 0) {
                                ...
                                $testing = "";
                                foreach ($stay_dates as $RsDates) {
                                    $checkratesdatas = $check_current_rate_all_datas_get->where("stay_date", $RsDates)->first();
                                    if ($checkratesdatas == "") {
                                        $newratedatas[] = collect($currentrates)->where("stay_date", $RsDates)->first();
                                    } else {
                                        $updatedatas = collect($currentrates)->where("stay_date", $RsDates)->first();
                                        $testing = CurrentRate::where("id", $checkratesdatas->id)->update($updatedatas);
                                    }
                                }

                                if (count($newratedatas) > 0) {
                                    $test_current_rate = CurrentRate::insert($newratedatas);
                                }
                        }
                        $log_data                 = [
                            "action_id"          => 2,
                ...
                } elseif ((!$this->googleHotelStatus($data['hotel_id'])) && (count($blocked_be_datas) > 0)) {
                    $current_filtered_dates = $filtered_dates;
                    $currents_cond_datas = [
                        "hotel_id"          => $hotel_id,
                        "room_type_id"        => $room_type_id,
                        "rate_plan_id"        => $rate_plan_id,
            ...
            $resp = array('status' => 0, 'response_msg' => 'No records found for this date', 'ota_name' => 'Bookingjini');
        }
        return $resp;
    }

    //Added by Jigyans dt : - 30-03-2023
    public function individualRateUpdate1111(Request $request)
    {
        $data = $request->all();
