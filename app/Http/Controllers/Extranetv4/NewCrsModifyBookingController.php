...
class NewCrsModifyBookingController extends Controller
{
    ...
    public function modifyBookingDates($hotel_id, $booking_id)
    {
        ...
        $bookingDetails =  Invoice::join('kernel.user_table', 'invoice_table.user_id', '=', 'user_table.user_id')
            ...
            ->leftjoin('kernel.image_table', 'image_table.image_id', '=', 'company_table.logo')
            ->select(
        ...
        if(isset($bookingDetails->image_name)){
            $logo = $bookingDetails->image_name;
        }else{
            $logo = '';
        }
        $booking_details['logo'] = "https://d3ki85qs1zca4t.cloudfront.net/bookingEngine/" . $logo; //default hotel logo else bookingjini logo
    ...
    public function saveModifyBookingDates(Request $request)
    {
        ...
        // if ($hotel_id == 1953 || $hotel_id == 2600) {
            $booking = [];
            $cart_data = [];
            $booking_details = Invoice::leftjoin('crs_booking', 'crs_booking.invoice_id', '=', 'invoice_table.invoice_id')
                ->where('invoice_table.invoice_id', $invoice_id)
                ->select('crs_booking.adult', 'crs_booking.no_of_rooms', 'invoice_table.room_type')
                ->first();

            $rate_plan_info = json_decode($booking_details->room_type);
            $rate_info = explode('(', $rate_plan_info[0]);
            ...
            $rate_plan = MasterRatePlan::where('hotel_id', $hotel_id)
                ->where('plan_type', $rate_plan_dtl)
                ->where('is_trash', 0)
                ->select('rate_plan_id')
                ->first();
            ...
            if ($room_details) {
                foreach ($room_details as $room_detail) {
                    $bookings['selected_adult'] = $booking_details['adult'];
                    $bookings['selected_child'] = '';
                    $bookings['rate_plan_id'] = $rate_plan_id;
                    $bookings['extra_adult_price'] = 0;
                    $bookings['extra_child_price'] = 0;
                    $bookings['bar_price'] = $room_detail['room_rate'];
                    $booking[] = $bookings;
                    $cart_data[] = array('rooms' => $booking, 'room_type_id' => $room_detail['room_type_id']);
                }
            }
        // }
        ...
        if ($room_details) {
            foreach ($room_details as $room_detail) {
                ...
                if ($room_rates <= 0) {
                    return response()->json(array('status' => 0, 'message' => 'Modification failed due to invalid Price'));
                } else {
                    continue;
                }
            }
        ...
        $booking_ref_no = Invoice::where('invoice_id', $invoice_id)->where('hotel_id', $hotel_id)->first();
        $cancel_previous_booking = $this->crsBookingsController->cancelBooking($invoice_id, 'modify');
        if ($cancel_previous_booking) {
            $delete_booking_allocation = DB::connection('bookingjini_cm')->table('bookings_allocation_log')->where('booking_id', $data['booking_id'])->delete();
            ...
            if ($room_details) {
                ...
                foreach ($room_details as $room_detail) {
                    ...
                    } elseif ($room_rates > 0 && $room_rates < 7500) {
                    ...
                    if (in_array($hotel_id, $hotel_info)) {
                        BeBookingDetailsTable::where('hotel_id', $hotel_id)->where('ref_no', $booking_ref_no->ref_no)->where('room_type_id', $room_type_id)->update(['room_rate' => $room_rates, 'tax_amount' => $gst_price]);
                    }
                }
                //---------------------update invoice_table-----------------------------------//
                $check_in_out = '[' . $modify_check_in . '-' . $modify_check_out . ']';
                $update_invoice = Invoice::where('invoice_id', $invoice_id)->where('hotel_id', $hotel_id)->update(['check_in_out' => $check_in_out, 'modify_date' => $modified_date, 'booking_status' => 1, 'modify_status' => 1, 'total_amount' => $total_price, 'paid_amount' => $total_price,  'tax_amount' => $total_gst_price]);
            ...
            if ($update_invoice && $update_htl_bkng && $update_crs_bkng) {
                ...
                $activity_id = "a24";
                $activity_description = "Modified CRS Booking id $booking_id ";
                $activity_from = "CRS";
                $user_id = 0;
    ...
    public function guestDetails($hotel_id, $booking_id)
    {
        ...
        $guestDetails =  Invoice::join('kernel.user_table', 'invoice_table.user_id', '=', 'user_table.user_id')
            ...
            ->select('user_table.first_name', 'user_table.last_name', 'user_table.company_name', 'user_table.email_id', 'user_table.address', 'user_table.mobile', 'user_table.GSTIN', 'crs_booking.payment_type', 'crs_booking.guest_name', 'crs_booking.internal_remark', 'crs_booking.guest_remark', 'crs_booking.modification_remark', 'crs_booking.booking_type')
            ->where('invoice_table.invoice_id', $invoice_id)
        ...
        if (isset($guestDetails['booking_type']) && $guestDetails['booking_type'] == 'group') {
    ...
    public function saveGuestDetails(Request $request)
    {
        ...
        // if ($hotel_id == 1953 || $hotel_id == 2600) {
            ...
            $cart_data = [];
            $booking_details = Invoice::leftjoin('crs_booking', 'crs_booking.invoice_id', '=', 'invoice_table.invoice_id')
                ->leftjoin('be_booking_details_table', 'be_booking_details_table.ref_no', '=', 'invoice_table.ref_no')
                ->where('invoice_table.invoice_id', $invoice_id)
                ->select('be_booking_details_table.room_type_id', 'be_booking_details_table.rate_plan_id', 'be_booking_details_table.rooms', 'crs_booking.adult', 'crs_booking.no_of_rooms', 'invoice_table.room_type')
                ->first();

            if ($room_details) {
                foreach ($room_details as $room_detail) {
                    $room_price = DB::table('rate_plan_log_table')
                                ->where('hotel_id', $hotel_id)
                                ->where('room_type_id', $room_detail['room_type_id'])
                                // ->where('rate_plan_id', $detail->rate_plan_id)
                    ...
                    $bookings['selected_adult'] = $booking_details['adult'];
                    $bookings['selected_child'] = '';
                    $bookings['rate_plan_id'] = $room_price->rate_plan_id;
                    $bookings['extra_adult_price'] = 0;
                    $bookings['extra_child_price'] = 0;
                    $bookings['bar_price'] = $room_price->bar_price;
                    $booking[] = $bookings;
                    $cart_data[] = array('rooms' => $booking, 'room_type_id' => $room_detail['room_type_id']);
                }
            }
        ...
        if ($updatedetails) {
            ...
            $bookingDetails['booking_type'] = $booking_type;
        ...
        if ($updatedetails && $bookingRes) {
            ...
            $is_ids = PmsAccount::where('name', 'IDS NEXT')->whereRaw('FIND_IN_SET(' . $hotel_id . ',hotels)')->first();
            if ($is_ids) {
                $inv_data = $this->bookingEngineController->handleIds($cart_data, $check_in, $check_out, $modified_date, $hotel_id, $users->user_id, 'Modify');
            }
