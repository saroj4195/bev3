    ...
    **/
    public function getInventeryByRoomTYpe(int $room_type_id ,string $date_from ,string $date_to,int $mindays)
    {
        ...
        {   $d=$date_from;
            ...
            $array=array('no_of_rooms'=>not_available,'block_status'=>1,'los'=>1,'room_type_id'=>$room_type_id,'date'=>$date_from,'day'=>$day);
        ...
        for($i=1;$i<=$diff; $i++ )
        {
            ...
            $inventory_details= $inventory
             ...
             if(empty($inventory_details))
             {
                ...
                // if(!empty($inv_rooms))
                // {
                    $array=array('no_of_rooms'=>not_available,
    ...
    public function getRatesByRoomnRatePlanOld(int $room_type_id, int $rate_plan_id, string $date_from, string $date_to)
    {
        ...
        for ($i = 1; $i <= $diff; $i++) {
            ...
            // $bar_price = ($rate_plan_details['bar_price'] == "") ? "0" : $rate_plan_details['bar_price'];
            // $multiple_occupancy = $rate_plan_details['multiple_occupancy'];
            // $bookingjini_price = ($rate_plan_details['bookingjini_price'] == "") ? "0" : $rate_plan_details['bookingjini_price'];
            // $extra_adult_price = ($rate_plan_details['extra_adult_price'] == "") ? "0" : $rate_plan_details['extra_adult_price'];
            // $extra_child_price = ($rate_plan_details['extra_child_price'] == "") ? "0" : $rate_plan_details['extra_child_price'];
            // $before_days_offer = $rate_plan_details['before_days_offer'];
            // $los = ($rate_plan_details['los'] == "") ? "0" : $rate_plan_details['los'];;
            ...
            $rate_plan_log_details = $rateplanlog
                ->select('bar_price', 'multiple_occupancy', 'multiple_days', 'block_status', 'extra_adult_price', 'extra_child_price', 'los')
                ->where(['room_type_id' => $room_type_id])
                ->where('rate_plan_id', '=', $rate_plan_id)
                ->where('from_date', '<=', $d)
                ->where('to_date', '>=', $d)
            ...
            if (empty($rate_plan_log_details)) {
                $array = array(
                    'bar_price' => ($bar_price == "") ? "0" : $bar_price,
                    'multiple_occupancy' => json_decode($multiple_occupancy),
                    'bookingjini_price' => ($bookingjini_price == "") ? "0" : $bookingjini_price,
                    'extra_adult_price' => ($extra_adult_price == "") ? "0" : $extra_adult_price,
                    'extra_child_price' => ($extra_child_price == "") ? "0" : $extra_child_price,
                ...
                if ($multiple_days != null) {
                    if ($multiple_days->$day == 0) {
                        ...
                        if (empty($rate_plan_log_details1[0])) {
                            ...
                            $los     = $rate_plan_log_details1[0]['los'];
                            if ($multiple_days1 != null) {
                                if ($multiple_days1->$day == 0) {
                                    $rate_plan_log_details2 = $rateplanlog
                                        ->select('bar_price', 'multiple_occupancy', 'block_status', 'extra_adult_price', 'extra_child_price', 'los')
                                        ->where(['room_type_id' => $room_type_id])
                                        ->where('rate_plan_id', '=', $rate_plan_id)
                                        ->where('from_date', '<=', $d)
                                        ->where('to_date', '>=', $d)
                                    ...
                                    if (empty($rate_plan_log_details2[0])) {
                                        ...
                                        $los     = $rate_plan_log_details2[0]['los'];
                                        $array = array(
                                            'bar_price' => $rate_plan_log_details2[0]->bar_price,
                                            'multiple_occupancy' => json_decode($rate_plan_log_details2[0]->multiple_occupancy),
                                            'bookingjini_price' => ($bookingjini_price == "") ? "0" : $bookingjini_price,
                                            'extra_adult_price' => ($rate_plan_log_details2[0]->extra_adult_price == "") ? "0" : $rate_plan_log_details2[0]->extra_adult_price,
                                            'extra_child_price' => ($rate_plan_log_details2[0]->extra_child_price == "") ? "0" : $rate_plan_log_details2[0]->extra_child_price,
                                            'before_days_offer' => $before_days_offer,
                                            'stay_duration_offer' => $stay_duration_offer,
                                            'lastminute_offer' => $lastminute_offer,
                                            'rate_plan_id' => $rate_plan_id,
                                    ...
                                    $array = array(
                                        'bar_price' => ($rate_plan_log_details1[0]->bar_price == "") ? "0" : $rate_plan_log_details1[0]->bar_price,
                                        'multiple_occupancy' => json_decode($rate_plan_log_details1[0]->multiple_occupancy),
                                        'bookingjini_price' => ($bookingjini_price == "") ? "0" : $bookingjini_price,
                                        'extra_adult_price' => ($rate_plan_log_details1[0]->extra_adult_price == "") ? "0" : $rate_plan_log_details1[0]->extra_adult_price,
                                        'extra_child_price' => ($rate_plan_log_details1[0]->extra_child_price == "") ? "0" : $rate_plan_log_details1[0]->extra_child_price,
                                        'before_days_offer' => $before_days_offer,
                                        'stay_duration_offer' => $stay_duration_offer,
                                        'lastminute_offer' => $lastminute_offer,
                                        'rate_plan_id' => $rate_plan_id,
                                ...
                                $array = array(
                                    'bar_price' => ($rate_plan_log_details['bar_price'] == "") ? "0" : $rate_plan_log_details['bar_price'],
                                    'multiple_occupancy' => json_decode($rate_plan_log_details['multiple_occupancy']),
                                    'bookingjini_price' => ($bookingjini_price == "") ? "0" : $bookingjini_price,
                                    'extra_adult_price' => ($rate_plan_log_details['extra_adult_price'] == "") ? "0" : $rate_plan_log_details['extra_adult_price'],
                                    'extra_child_price' => ($rate_plan_log_details['extra_child_price'] == "") ? "0" : $rate_plan_log_details['extra_child_price'],
                                    'before_days_offer' => $before_days_offer,
                                    'stay_duration_offer' => $stay_duration_offer,
                                    'lastminute_offer' => $lastminute_offer,
                                    'rate_plan_id' => $rate_plan_id,
    ...
    public function getBookingByRoomtype($hotel_id,$room_type_id,$date_from,$date_to)
    {
        ...
        $booking=new Booking();
        // $date1 = date_create(date($date_from));
        // $date2=date_create(date($date_to));
         $date1 = date_create($date_from);
        $date2=date_create($date_to);
        $diff=date_diff($date1,$date2);
        $diff=$diff->format("%a");
    ...
    {       
        ...
        if($room_types){
            foreach($room_types as $room_type){ 
                ...
                $rate_plans_data = array_unique($rate_plans_data);
                sort($rates_plan_data, SORT_NUMERIC );
                if($hotel_id == 3065){
                    var_dump($rates_plan_data);
                }
