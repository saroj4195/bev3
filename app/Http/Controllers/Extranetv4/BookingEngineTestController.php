...
**/
    public function getRoomDetails(string $api_key,int $hotel_id,int $room_type_id,Request $request)
    {
        ...
        $room_types=MasterRoomType::select('room_type_id','room_type','description','image','room_amenities','bed_type','room_size_value','room_size_unit','room_view_type')->where($conditions)->first();
...
//Create Invoice
 ...
 public function createInvoice($hotel_id,$cart,$coupon,$paid_services,$check_in,$check_out,$user_id,$percentage,$ref_no,$refund_protect_price,$rp_member_id,$source)
 {
    if($hotel_id == 2065  || $hotel_id == 2881  || $hotel_id == 2882  || $hotel_id == 2883  || $hotel_id == 2884  || $hotel_id == 2885  || $hotel_id == 2886 ){
     ...
     <td style="border: 1px #0; padding: 4%; border-style: double;">
         <table width="100%" border="0">
             ...
             <tr><td></td><td style="font-size: 16px;font-weight: bold;" align="right">PAYMENT REFERENCE NUMBER : '.$transaction_id.'</td></tr>
             <tr>
     ...
     if($source  == 'GEMS'){
        ...
        <td align="center">'.$currency_code.'<span id="pd_amt">'.$due_amount.'</span></td>
        </tr>
        <tr>
            <td colspan="7" align="right">Pay at hotel&nbsp;&nbsp;</td>
            <td align="center">'.$currency_code.'<span id="du_amt">'.$paid_amount.'</span></td>
        </tr>
        <tr>
            <td colspan="8"><p style="color: #ffffff;">* </p></td>
        </tr>';
...
public function successBooking($invoice_id, $mihpayid, $payment_mode, $hash, $txnid)
{
    ...
    if($mihpayid != "true" && $mihpayid != "crs" && $mihpayid != "quickpayment" && $mihpayid !='modify'){
    ...
    if(1)
    {
        ...
        $inv = $this->invoiceMail($invoice_id);
        if($mihpayid != "true" && $mihpayid != "crs" && $mihpayid != "quickpayment" && $mihpayid !='modify'){
            $inv = $this->invoiceMail($invoice_id);
        }
    ...
    if(1)
    {
        ...
        {
            if($company_id == 2565){
                return "http://romtbooking.bookingjini.tech";
            }
            else{
...
public function getTaxDetails(string $company_id,string $hotel_id,Request $request){
    ...
    **/
    ...
    for($currentDate = $begin; $currentDate < $end; $currentDate += (86400)){
        ...
        valid_to,discount_type,coupon_for,discount,blackoutdates,a.date,a.abc
        FROM
        (
        select t2.coupon_id,
        case when t2.room_type_id!=0 then t2.room_type_id else 0 end as room_type_id,
        coupon_name,coupon_code,valid_from,valid_to,discount_type,coupon_for,discount,blackoutdates,"'.$Store.'" as date,
        case when "'.$Store.'" between valid_from and valid_to then "yes" else "no" end as abc
        ...
        FROM booking_engine.coupons where hotel_id = "'.$hotel_id.'" and coupon_for = 1 and is_trash = 0 and ("'.$Store.'" between valid_from and valid_to) 
        GROUP BY room_type_id
        order by coupon_id desc
        ) t2 ON coupons.room_type_id = t2.room_type_id AND coupons.coupon_id = t2.coupon_id
        ) AS a where a.abc = "yes"
        order by room_type_id,coupon_id desc'));
            ...
            $status[] = $data->status;
            $b_dates = $data->blackoutdates;
            $blackoutdates = json_decode($b_dates);
            if (in_array($Store, $blackoutdates)) {
               // $retrive_data = Coupons::where('c0oupon_id', $get_data['coupon_id'], )->
            } else {
...
{
    $booking_status='Commit';
    $winhms_status=$this->PmsController->getwinhmsStatus($hotel_id);
    if ($winhms_status) {
        $winhms_data=$this->prepare_winhms_data($cart, $from_date, $to_date, $booking_date, $hotel_id);
        $customer_data=$this->getUserDetails($user_id)->toArray();
        $type="Bookingjini";
        $last_winhms_id=$this->PmsController->winhmsBookings($hotel_id, $type, $winhms_data, $customer_data, $booking_status);
        if ($last_winhms_id) {
...
public function pushBookingToGems($invoice_id,$gems){
  ...
  ->select('hotel_booking.user_id', 'invoice_table.room_type', 'invoice_table.ref_no', 'invoice_table.extra_details', 'invoice_table.booking_date', 'invoice_table.invoice_id', 'invoice_table.total_amount', 'invoice_table.paid_amount', 'invoice_table.hotel_name', 'invoice_table.hotel_id','company_table.currency','invoice_table.tax_amount','invoice_table.discount_amount','invoice_table.booking_status','invoice_table.modify_status')
        ...
        foreach ($getRoomDetails as $rm_key => $getRoom) {
            // if($bk_details->hotel_id  == 2319){
                ...
                for($i=0; $i<$total_rooms; $i++){
                    ...
                    $total_room_rate+= (double)$room_rate_info[$i];
                    $total_gst_amount+= (double)$tax_amount_info[$i];
                    $extra_adult_amount+= (double)$extra_adult_info[$i];
                    $extra_child_amount+= (double)$extra_child_info[$i];
...
public function fetchCancelRefundAmount($invoice_id){
 ...
 //API to fetch BE Notifications Popup 
    ...
    public function checkRoomDates(){
        $result = DB::table('kernel.room_rate_plan')
        ->join('kernel.room_type_table','room_type_table.room_type_id', '=','room_rate_plan.room_type_id')
        ->join('kernel.rate_plan_table','rate_plan_table.rate_plan_id', '=','room_rate_plan.rate_plan_id')
        ->join('kernel.hotels_table','hotels_table.hotel_id', '=','room_rate_plan.hotel_id')
        ->select('room_rate_plan.from_date','room_rate_plan.to_date','room_type_table.room_type','rate_plan_table.plan_name','hotels_table.hotel_name')
        // ->where('room_rate_plan.hotel_id','1953')
        ->where('room_rate_plan.to_date','>=','2022-10-28')
        ->where('room_rate_plan.to_date','<=','2023-01-01')
        ->get();
        ...
        {
            $html .= '<tr>';
            $html .= '<td>'.$record->from_date.'</td>';
            $html .= '<td>'.$record->to_date.'</td>';
            $html .= '<td>'.$record->room_type.'</td>';
            $html .= '<td>'.$record->plan_name.'</td>';
            $html .= '<td>'.$record->hotel_name.'</td>';
            $html .= '</tr>';
        }
        $html .= '</table>';
        echo $html;
