...
class BookingEngineInvRateController extends Controller
{
        ...
        foreach ($imp_data['ota_id'] as $otaid) {
            if ($otaid == -1) { //booking engine update
                ...
                foreach ($inventory['inv'] as $inv) {
                    $inv_update_date[] = date('Y-m-d', strtotime($inv['date']));
                }

                $check_block_status = DynamicPricingCurrentInventory::where('hotel_id', $imp_data['hotel_id'])
                    ->where('ota_id','-1')
                    ->where('room_type_id', $inventory['room_type_id'])
                    ->whereIn('stay_day', $inv_update_date)
                    ->pluck('block_status','stay_day');
                ...
                // foreach ($inventory as $invs) {
                    foreach ($inventory['inv'] as $inv) {
                        ...
                        $formate_date = date('Y-m-d', strtotime($inv['date']));

                        $inv['room_type_id']       = $inventory['room_type_id'];
                        $inv['block_status']       = isset($check_block_status[$formate_date])?$check_block_status[$formate_date]:0;
    ...
    //Added by Jigyans dt : - 08-08-2023
