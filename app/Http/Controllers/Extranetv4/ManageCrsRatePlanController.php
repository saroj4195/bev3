...
dd("hii");
$room_rate_plan_details = $roomRateSettings->join('bookingjini_kernel.rate_plan_table as a', 'roomrate_settings.rate_plan_id', '=', 'a.rate_plan_id')
->join('bookingjini_kernel.room_type_table as b','roomrate_settings.room_type_id', '=', 'b.room_type_id')
...
->where('to_date' , '>=' , $d)
->first();
if(sizeof($room_rate_plan_details)==0)//If Room rate plans Not with in date range then Latest created room rate plan is considered
{
...
$userModal=new AdminUser();
$user=$userModal->where('admin_id',$user_id)->select('role_id')->first();
