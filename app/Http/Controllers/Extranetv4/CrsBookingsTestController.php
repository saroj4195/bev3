...
class CrsBookingsTestController extends Controller
{
    ...
    public function getNumberWiseUser(Request $request)
    {
        ...
        $getUserInfo = User::select('first_name', 'last_name', 'email_id', 'company_name', 'address', 'GSTIN')
            ...
            if(!empty($getUserInfo)){
                if(empty($getUserInfo->last_name)){
                    $getUserInfo['last_name'] = 'NA';
                }
            }
    ...
    public function filterURL(Request $request)
    {
        ...
        $url = parse_url($url_details);

        if (isset($url['scheme']) == 'https' || isset($url['scheme']) == 'http') {
            $host = preg_replace('#^www\.(.+\.)#i', '$1', $url['host']);
        } else {
    ...
    public  function listViewBookings(Request $request)
    {
        ...
        if (sizeof($be_data) > 0) {
            foreach ($be_data as $be) {
                ...
                $fetchHotelLogo = DB::table('kernel.image_table')->where('image_id', $be->logo)->select('image_name')->first();
    ...
    public function sourceList($hotel_id)
    {
        ...
        if ($app_name) {
            ...
            foreach ($app_name as $be_source) {
                if ($be_source == 'JINI BOOK') {
    ...
    public function channelLogo($hotel_id)
    {
        ...
        $ota_logo['Amadeus'] = '';
        $ota_logo['Onlinevacations'] = '';
        $ota_logo['OTDC'] = '';
        $ota_logo['ClearTripHyperGuest'] = '';
