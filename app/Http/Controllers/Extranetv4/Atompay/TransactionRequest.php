<?php
namespace App\Http\Controllers\Extranetv4\Atompay;
use App\Http\Controllers\Extranetv4\Atompay\AtomAES;
...
class TransactionRequest extends Controller
{
    ...
    public function getPGUrl(){
        if ($this->mode != null && $this->mode != "") {
            try {
                $data = $this->getData();
                //$this->writeLog($data);
                return $this->transactionUrl."?".trim($data);
            } catch ( Exception $ex ) {
    ...
    private function writeLog($data){
        ...
        $fp = fopen("Atompay/log/".$fileName, 'a+');
